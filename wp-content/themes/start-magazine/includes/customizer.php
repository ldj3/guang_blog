	...
	$wp_customize->add_section(
		new Start_Magazine_Upsell_Section( $wp_customize, 'custom_theme_upsell',
			array(
				...
				'pro_url'  => 'https://axlethemes.com/downloads/start-magazine-pro/',
				'priority' => 1,
			)
		)
	);
...
function start_magazine_customize_controls_register_scripts() {
	...
	wp_enqueue_script( 'start-magazine-customize-controls', get_template_directory_uri() . '/js/customize-controls' . $min . '.js', array( 'jquery', 'customize-controls' ), '1.0.0', true );
	wp_enqueue_style( 'start-magazine-customize-controls', get_template_directory_uri() . '/css/customize-controls' . $min . '.css', array(), '1.0.0' );
